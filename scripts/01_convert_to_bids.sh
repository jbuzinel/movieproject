...
echo "This script was developed and tested under bash 5.0.17(1)-release"
echo -e "Your version of bash is $BASH_VERSION\n"

# Heudiconv
echo "This script was developed and tested under heudiconv 1.3.0"
...
## Run from /MovieProject/ folder

# Define path to the project
project_path="/nndb_teens/jure/MovieProject"
subjects="XXX"

# Excluded (?) make a note if any are excluded...

for subj_id in $subjects; do
  subj_dir="${project_path}/raw_data/sub-$subj_id/"

  (
  echo "Checking directory: '$subject_dir'"
  ls -ld "$subj_dir"
  ...
  if [ -d "$subj_dir" ]; then
    ...
    for full_sess_id in $sess_ids; do
      sess_id="${full_sess_id:5}" # Assumes each participant has a folder in the sess- format and just takes the number
      echo "Processing session: $sess_id"
...
## Review dicominfo.tsv table and create/edit heuristic.py file. Heuristic file is going to convert all raw dicom files to BIDS-valid format
  ...
  if [ -d "$subj_dir" ]; then
    ...
    for full_sess_id in $sess_ids; do
      ...
      docker run --rm -v ${PWD}:/base nipy/heudiconv:latest \
       ...
       -o /base/bids_data/ \
       -f /base/analysis/heuristic_sess_"${sess_id}".py \
       -s "$subj_id" -ss "${sess_id}" \
       -c dcm2niix -b
...
echo "Changing the owner of bids_data folder from root to jbuzinel and grant permissions to edit files... "
sudo chown -R jbuzinel:jbuzinel "${project_path}/bids_data"
...
for subject in $subjects; do
    for session in 001 002; do
        python add_intendedfor_field.py "${project_path}/bids_data" "$subject" "$session"
    done
done
...
echo "Changing the owner of bids_data folder from root to jbuzinel and grant permissions to edit files..."
chown -R jbuzinel:jbuzinel "${project_path}/bids_data"
